...
def test_html_element_valid_construction() -> None:
...
def test_html_element_defaults() -> None:
    element = HtmlElement(**_html_element())
...
def test_missing_html_rejected() -> None:
    data = _html_element()
    del data["html"]
...
def test_image_element_extra_field_rejected() -> None:
    with pytest.raises(ValidationError, match="Extra inputs"):
        ImageElement(**_image_element(html="sneaky"))


def test_markdown_element_valid_construction() -> None:
...
def test_missing_markdown_rejected() -> None:
    data = _md_element()
    del data["markdown"]
    with pytest.raises(ValidationError):
...
def test_mermaid_element_valid_construction() -> None:
...
def test_missing_mermaid_rejected() -> None:
...
def test_auto_auto_rejected() -> None:
...
def test_is_slide_ir() -> None:
    assert issubclass(SlideIR, SlideIR)


def test_slide_ir_valid_construction() -> None:
...
def test_slide_ir_defaults() -> None:
    slide = SlideIR(**_slide())
    assert slide.initial_scroll_position == 0
    assert slide.scroll_speed == 1.0
    assert slide.easing == "linear"
...
def test_initial_scroll_position_beyond_range_rejected() -> None:
...
def test_multiple_element_types() -> None:
    slide = SlideIR(
        **_slide(
            elements=[
...
def test_element_with_animated_opacity() -> None:
...
def test_returns_slide_ir(tmp_path: Path) -> None:
...
def test_asset_paths_resolved_to_absolute(tmp_path: Path) -> None:
...
def test_slide_type_property(tmp_path: Path) -> None:
    src = _write(tmp_path, "s.slide.json", MINIMAL_JSON5)
    ir = SlideIR.from_file(src)
...
def test_scroll_range_defaults_to_auto() -> None:
    # --- arrange / act ----------------------
...
def test_scroll_range_accepts_explicit_auto() -> None:
    # --- arrange / act ----------------------
...
def test_scroll_range_accepts_numeric() -> None:
    # --- arrange / act ----------------------
...
def test_scroll_range_rejects_negative_number() -> None:
    # --- arrange / act / assert -------------
...
def test_scroll_range_rejects_unknown_string() -> None:
    # --- arrange / act / assert -------------
    with pytest.raises(ValidationError):
        SlideIR(title="T", scroll_range="huge", elements=[_html_element()])


def test_auto_skips_initial_scroll_position_upper_bound() -> None:
    # --- arrange / act ----------------------
...
def test_auto_skips_snap_position_upper_bound() -> None:
    # --- arrange / act ----------------------
    ir = SlideIR(
        title="T",
        scroll_range="auto",
...
def test_auto_still_rejects_negative_snap_positions() -> None:
    # --- arrange / act / assert -------------
...
def test_font_scale_defaults_to_one() -> None:
    # --- arrange / act ----------------------
...
def test_font_scale_accepts_positive_values() -> None:
    # --- arrange / act ----------------------
...
def test_font_scale_rejects_zero_and_negative() -> None:
    # --- arrange / act / assert -------------
